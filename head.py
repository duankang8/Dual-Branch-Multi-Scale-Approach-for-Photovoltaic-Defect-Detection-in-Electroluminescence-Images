...
class Detect(nn.Module):
    ...
    def __init__(self, nc=12, ch=()):
        ...
        # self.cv3 = nn.ModuleList(nn.Sequential(      #全连接层去做分类
            # nn.AdaptiveAvgPool2d((1, 1)),  # Global average pooling
            # nn.Flatten(),  # Flatten the output to (batch_size, c3)
            # nn.Linear(x, c3),  # Fully connected layer
            # nn.ReLU(inplace=True),  # Activation function
             # nn.Linear(c3, self.nc)  # Output layer for classification
        # ) for x in ch)
...
class Segment(Detect):
    ...
    def __init__(self, nc=12, nm=32, npr=256, ch=()):
...
class OBB(Detect):
    ...
    def __init__(self, nc=12, ne=1, ch=()):
...
class Pose(Detect):
    ...
    def __init__(self, nc=12, kpt_shape=(17, 3), ch=()):
...
class WorldDetect(Detect):
    def __init__(self, nc=12, embed=512, with_bn=False, ch=()):
